CREATE table `Избранные резюме`(`ID` int PRIMARY KEY, 
                                `iD организации` int, 
                                `ID резюме` int
);

CREATE table `резюме`(`ID резюме` int PRIMARY KEY, 
                      `ID пользователя` int, 
                      `Профессия` int,
                      `Пол`varchar(15),
                      `Дата рождения`date,
                      `Город проживания`int,
                      `Образование`varchar(100),
                      `Желаемая З/П` DECIMAL (11.2), 
                      `О себе` varchar(255)
);

CREATE table `Город` (`ID города` int PRIMARY KEY,
                      `Название города` varchar(100)
);

CREATE table `Организация` (`ID организации` int PRimary KEY,
                            `Название` varchar(100), 
                            `Город` int,
                            `Информация` varchar(255), 
                            `Телефон` varchar(11), 
                            `E-mail` varchar(100)
);

CREATE table `Пользователь` (`ID пользователя` int PRimary KEY,
                             `Фамилия` varchar(100),
                             `Имя` varchar(100),
...
CREATE table `Вакансия` (`ID вакансии` int PRimary KEY,
                         `Название вакансии` varchar(100),
                         `Профессия` int,
                         `Заработная плата` decimal (11.2), 
                         `Город` int,`Организация` int,
...
CREATE table `Избранные вакансии` (`ID` int PRIMARY KEY,
                                   `ID пользователя` int,
                                   `Вакансия` int
);

CREATE table `Список профессий` (`ID профессии` int PRIMARY KEY,
                                 `Название` varchar(100)
);

CREATE table `Users` (Number int PRIMARY KEY,
                      Login varchar(100),
...
ALTER TABLE `избранные резюме` ADD INDEX(`iD организации`, 
                                         `ID резюме`);

ALTER TABLE `резюме` ADD INDEX( `профессия`, 
                                `Город проживания`);

ALTER TABLE `Организация` ADD INDEX(`Город`);

ALTER TABLE `вакансия` ADD INDEX(`Профессия`, 
                                 `Город`, 
                                 `Организация`);

ALTER TABLE `пользователь` ADD INDEX(`город проживания`);

ALTER TABLE `избранные вакансии` ADD INDEX(`ID пользователя`, 
                                           `Вакансия`);
